...
/**
 * Front camera for surprise POV selfies during play.
 * Sits tiny in a corner so gameplay stays clear; snaps at random intervals.
 */
export function CatCam({ enabled, paused, mode, onSnap }: Props) {
  ...
  useEffect(() => {
    if (!enabled || Platform.OS === 'web' || !permission?.granted) return;
    let cancelled = false;
